...
import sys
from config import STRATEGY_PARAMS, LOG_FILE, RESULTS_DIR, INITIAL_CASH, COMMISSION, SHUFFLE_STOCKS, PLOT_RESULTS
...
def main():
    ...
    try:
        ...
        # Initialize strategy
        strategy = PercentageChangeStrategy(STRATEGY_PARAMS)
        ...
        if successful_results:
            sorted_results = sorted(successful_results, key=lambda x: x['return'])
            lowest_return = sorted_results[0]
            highest_return = sorted_results[-1]
            
            # Re-run backtests with plotting for highest and lowest return stocks if PLOT_RESULTS is True
            if PLOT_RESULTS:
                logger.info(f"Plotting results for highest return stock {highest_return['symbol']} and lowest return stock {lowest_return['symbol']}")
                
                # Plot highest return stock
                high_strategy = MovingAverageStrategy(**STRATEGY_PARAMS.get('moving_average', {}))
                high_backtest = BacktestEngine(stock_data[highest_return['symbol']], high_strategy, highest_return['symbol'])
                high_backtest.run(plot=True)
                
                # Plot lowest return stock
                low_strategy = MovingAverageStrategy(**STRATEGY_PARAMS.get('moving_average', {}))
                low_backtest = BacktestEngine(stock_data[lowest_return['symbol']], low_strategy, lowest_return['symbol'])
                low_backtest.run(plot=True)
